...
void MapFacade::buildWay(Personage *pers, QPoint point)
{
	...
	QList<Personage *>::iterator i;
	for (i = listOfPersonages.begin(); i != listOfPersonages.end(); ++i)
		if ((*i)->currentPos() != point)
			aStarCells[(*i)->currentPos().x()][(*i)->currentPos().y()]->walkable = false;
	else
		if ((*i)->getPersFraction() == pers->getPersFraction())
			return;
